 ...
 * @author USUARIO
 */
public class LogCocinero implements Serializable {
    ...
    public Cocinero BuscarCocinero(String cedula) {
        List<Factura> ListaFacturas = objLogFactura.findFacturaEntities();
        List<Pedido> ListaPedido = objLogPedido.findPedidoEntities();
        List<Cocinero> ListaCocinero = findCocineroEntities();
        
        Collection<Pedido> collectionPedido = new ArrayList<Pedido>();
        Collection<Factura> collectionFactura = new ArrayList<Factura>();
         Cocinero resultado = new Cocinero();

        for (Cocinero auxc : ListaCocinero) {
            if (auxc.getCedula().equals(cedula)) {
                resultado = auxc;
            }
//            System.out.println(resultado);
        }
        
        for (Pedido auxp : ListaPedido) {
            if (auxp.getIdCocinero().getIdCocinero()==resultado.getIdCocinero()) {
                collectionPedido.add(auxp);
                
            }
//            System.out.println(auxp);
        }
        
        resultado.setPedidoCollection(collectionPedido);
        return resultado;
